...
Experiments comparing sketch methods (subsampling and count) for increasing
momentum with heuristic versus CG and direct solver

Section 9.4 in the LaTex
...
```
$ python experiments/compare_conjugate_gradient.py boston -k True -r 1e-6 -t 1e-4 -m 2000 -n 3
```
"""
...
def load_model(
    ...
    if solver in ["direct", "cg"]:
        # momentum does not exist for direct solver and cg
        algo_mode = "auto"

    # if solver in ["gaussian", "hadamard"]:
...
@click.option(
    ...
    default=True,
    type=bool,
    help="use heuristic for increasing momentum",
)
@click.option(
...
def compare_cg(
    ...
    sketch_size = int(round(m ** (2 / 3)))
    sketch_size_formula = "m^(2/3)"
    sketch_size_formula_reformatted = "m_power_2_over_3"

    print(f"sketch size: {sketch_size_formula} = {sketch_size}\n")

    exp_9_4 = ExperimentSketches(
        dataset.name,
        X,
        y,
        regularizer,
    ...
    exp_9_4.run_full_exp(verbose=False)
    ...
    if save:
        ...
        exp_name = f"{exp_name}_compare_cg_all_sketches"

        try:
            folder_path = os.path.join(os.getcwd(), "experiments/results/9_4_cg_plots")
        except FileNotFoundError:
            folder_path = "./experiments/results/9_4_cg_plots"
        exp_9_4.save(exp_name, folder_path)
