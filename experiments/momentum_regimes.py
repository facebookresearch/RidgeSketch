...
class ExperimentMomentum:
    ...
    def save_plots(self, exp_name, save_path):
        for i, sketch_size in enumerate(self.sketch_sizes):
            sketch_size_formula = self.sketch_size_formulas[i]
            for solver in self.solvers:
                print(f"sketch size / solver = {sketch_size} / {solver}")
                self.plot_residuals(
                    solver,
                    sketch_size,
                    sketch_size_formula,
...
def momentum_regimes(
    ...
    sketch_sizes = [m ** (2 / 3), m / 8, m / 4]
    sketch_sizes = [int(round(x)) for x in sketch_sizes]
    sketch_size_formulas = ["m^(2/3)", "m/8", "m/4"]

    # sketch_sizes = [m ** (2 / 3), m / 4]
    # sketch_sizes = [int(round(x)) for x in sketch_sizes]
    # sketch_size_formulas = ["m^(2/3)", "m/4"]

    # sketch_sizes = [m / 4, m / 2]
    # sketch_sizes = [int(round(x)) for x in sketch_sizes]
    # sketch_size_formulas = ["m/4", "m/2"]

    # sketch_sizes = [np.sqrt(m), m ** (2 / 3), m / 2]
    # sketch_sizes = [int(round(x)) for x in sketch_sizes]
    # sketch_size_formulas = ["sqrt(m)", "m^(2/3)", "m/2"]

    # sketch_sizes = [int(round(m / 2)]
    # sketch_size_formulas = ["m/2"]

    # sketch_sizes = [int(round(m / 4))]
    # sketch_size_formulas = ["m/4"]

    # sketch_sizes = [int(round(m ** (2/3)))]
    # sketch_size_formulas = ["m^(2/3)"]
